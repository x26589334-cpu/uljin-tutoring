# 울진과외 (uljin-tutoring) — 작업 규칙

경상북도 울진군 지역 타겟 과외 사이트. 정적 사이트, 빌드 없음.
...
- 검색엔진 소유확인: 네이버 `1bf9c137c2e690a98f4a2f428e3b06663b15d941` · 구글 `iTap7ga4pUy0S9kUAslc_fKngaCS7ul2Zk028LXfehM` — 홈 `<head>` 에 생성기가 넣는다(`regions.js` 의 `verify`). **`index.html` 에 손으로 넣지 말 것** — 재생성하면 사라진다. 코드를 바꾸려면 `verify` 를 고치고 재생성.
...
5. `& "$env:USERPROFILE\사이트관리\로그기록.ps1" -Repo uljin-tutoring -Status 완료 -What "글 N편"`
